...
class LAMDData : public nodelet::Nodelet
{
public:
  LAMDData() = default;
  ~LAMDData() override = default;
...
private:
  ...
  ros::Publisher  pub_raw_encoder_data;
  ...
  void llcpCallback(const mrs_modules_msgs::Llcp::ConstPtr& msg) {
    ...
    nav_msgs::Odometry raw_encoder_state_odom;
    raw_encoder_state_odom.header.stamp              = ros::Time::now();
    ...
    CAM_R_y << 
      std::cos(static_cast<double>(encoder_1_in_rad_)), 0, std::sin(static_cast<double>(encoder_1_in_rad_)),
      0, 1, 0,
    -std::sin(static_cast<double>(encoder_1_in_rad_)), 0, std::cos(static_cast<double>(encoder_1_in_rad_));
    ...
    q = Matrix_vector_mul(R_curr,Matrix_vector_mul(CAM_R_y,e_3_neg));
    q = sat_q(q);
...
// /- Publish link state data  --------------------------------------
    nav_msgs::Odometry Link_state_odom;
    Link_state_odom.header.stamp              = ros::Time::now();

// /- Before implementing the filter-  ---------------------------------
    Link_state_odom.pose.pose.orientation.x  = q[0];
    Link_state_odom.pose.pose.orientation.y  = q[1];
    Link_state_odom.pose.pose.orientation.z  = q[2];
...
// /- After implementing the filter-  ---------------------------------
    Link_state_odom.pose.pose.position.x     = q[0];
    Link_state_odom.pose.pose.position.y     = q[1];
    Link_state_odom.pose.pose.position.z     = q[2];
// --------------------------------------------------------------------
    ...
    q_old = q;
// /- Before implementing the filter-  ---------------------------------
    Link_state_odom.twist.twist.angular.x = q_dot[0];
    Link_state_odom.twist.twist.angular.y = q_dot[1];
    Link_state_odom.twist.twist.angular.z = q_dot[2];
    // --- Apply low-pass filters ---
    ...
    q_dot = sat_q_dot(q_dot);
// /- After implementing the filter-  ---------------------------------
    Link_state_odom.twist.twist.linear.x = q_dot[0];
    Link_state_odom.twist.twist.linear.y = q_dot[1];
    Link_state_odom.twist.twist.linear.z = q_dot[2];

    // Link_state_odom.pose.pose.orientation.roll     = encoder_1_in_rad_;
    // Link_state_odom.pose.pose.orientation.pitch    = encoder_2_in_rad_;
// --------------------------------------------------------------------
  ...
  // | ----------------- Publish Link States ----------------- |
    pub_link_state.publish(Link_state_odom);
...
void onInit() override {
  ...
  sub_raw_CAM_data  = nh.subscribe(topic_name_CAM_device_raw, 10, &LAMDData::llcpCallback, this);
  sub_IMU_data      = nh.subscribe(topic_name_IMU_raw, 10, &LAMDData::imu_data_callback, this);
...
// Register nodelet
PLUGINLIB_EXPORT_CLASS(mrs_llcp_ros::LAMDData, nodelet::Nodelet)
